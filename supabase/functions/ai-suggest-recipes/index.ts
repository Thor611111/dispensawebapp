...
Deno.serve(async (req) => {
  ...
  try {
    const { foodItems, preferences, count = 5, likes = [], dislikes = [], filters = {}, slotsPlan = [], recentTitles = [], viewedTitles = [], frequentIngredients = [], query = "", searchIngredients = [], mode = "personalized" } = await req.json();
    ...
    const isExplore = mode === "explore" || query || (searchIngredients as string[]).length > 0;
    const exploreBlock = isExplore
      ? `\n\nMODALITÀ ESPLORA: l'utente sta cercando ricette generali, NON limitate alla sua dispensa attuale.${query ? `\n- Cerca ricette che corrispondano a: "${query}" (nome del piatto, tipologia o cucina).` : ""}${(searchIngredients as string[]).length ? `\n- Le ricette DEVONO includere come ingredienti principali: ${(searchIngredients as string[]).join(", ")}.` : ""}\n- Proponi ricette classiche o creative anche se richiedono ingredienti non in dispensa.\n- Se possibile, indica nel 'reason' quali ingredienti l'utente ha già e quali mancano.`
      : "";
    const pantryBlock = isExplore
      ? `Dispensa attuale (riferimento opzionale):\n${(foodItems ?? []).map(fmtFood).join("\n") || "(vuota)"}`
      : `Dispensa attuale:\n${(foodItems ?? []).map(fmtFood).join("\n") || "(vuota)"}`;
    const priorityBlock = isExplore
      ? `\n\nRispetta diete/allergie e i filtri richiesti.`
      : `\n\nPriorità: prima gli alimenti in scadenza, poi quelli mai usati o non usati da tempo. Personalizza fortemente sui gusti dell'utente: stile dei piatti graditi/visti, ingredienti ricorrenti, evita categoricamente i non graditi.`;
    const ctx = `Data odierna: ${today.toISOString().slice(0,10)} · Stagione: ${season}\n\n${pantryBlock}\n\nPreferenze:\n- Persone: ${preferences?.household_size ?? 2}\n- Diete: ${(preferences?.diets ?? []).join(", ") || "nessuna"}\n- Allergie: ${(preferences?.allergies ?? []).join(", ") || "nessuna"}\n- Obiettivi: ${(preferences?.goals ?? []).join(", ") || "—"}\n- Budget settimanale: ${preferences?.weekly_budget ?? "non impostato"} EUR\n- Ricette gradite (proponi simili per stile/ingredienti): ${likes.join(", ") || "—"}\n- Ricette NON gradite (EVITA assolutamente piatti simili): ${dislikes.join(", ") || "—"}\n- Ricette aperte/lette di recente dall'utente (segnale di interesse, ispira piatti simili ma NON identici): ${(viewedTitles as string[]).join(", ") || "—"}\n- Ingredienti più ricorrenti nelle ricette salvate (privilegia questi gusti): ${(frequentIngredients as string[]).join(", ") || "—"}\n- Ricette già proposte di recente (NON ripetere): ${recentTitles.join(", ") || "—"}${exploreBlock}${priorityBlock}${filterLines.length ? `\n\nFiltri OBBLIGATORI:\n${filterLines.join("\n")}` : ""}${slotPlanLines ? `\n\nDevi proporre ESATTAMENTE ${targetCount} ricette, una per ciascuno di questi slot in ordine. Compila SEMPRE il campo assigned_to con date e slot esatti:\n${slotPlanLines}\n\nRegole:\n- Colazione: dolce o salata leggera, veloce.\n- Pranzo: piatto unico/primo nutriente.\n- Cena: più leggera, secondo+contorno o zuppa.\n- Spuntino: snack semplice.\n- Alterna proteine (carne, pesce, legumi, uova, vegetariano) e tipologie (pasta, riso, zuppa, insalata, secondo+contorno).\n- Rispetta la stagione (${season}) con ingredienti tipici italiani di stagione.\n- NON ripetere ricette tra gli slot.` : ""}`;
